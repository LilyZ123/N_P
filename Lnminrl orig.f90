subroutine nminrl
          ...
          ! Allocate memory for variables declared in the parm module
          allocate(cmn(NSOIL))
          allocate(idplt(NSOIL))
          allocate(rsdco_pl(NSOIL))
          allocate(sol_aorgn(NSOIL,j))
          allocate(sol_cbn(NSOIL,j))
          allocate(sol_fon(NSOIL,j))
          allocate(sol_fop(NSOIL,j))
          allocate(sol_no3(NSOIL,j))
          allocate(sol_orgn(NSOIL,j))
          allocate(sol_orgp(NSOIL,j))
          allocate(sol_rsd(NSOIL,j))
          allocate(sol_st(NSOIL,j))
          allocate(sol_tmp(NSOIL,j))
          allocate(sol_solp(NSOIL,j))
          allocate(sol_ul(NSOIL,j))
          allocate(sol_fc(NSOIL,j))
          allocate(sdnco(j))
          ...
          rsdco_pl(:) = 0.0
          sol_aorgn(:,:) = 0.0
          sol_cbn(:,1) = (/2.61, 0.25, 0.11, 0.07/)
          sol_fon(:,:) = 53.0
          sol_fop(:,:) = 53.0 ! need to find values to replace
          sol_no3(:,:) = 67.0
          sol_orgn(:,:) = 124.0
          sol_orgp(:,:) = 124.0 ! need to find values to replace
          sol_rsd(:,:) = 0.0
          sol_st(:,:) = 1.0 ! need values to replace
          sol_tmp(:,:) = 10.0 ! need values to replace
          sol_solp(:,:) = 18.75 ! need values to replace
          sol_ul (:,:) = 15.0 ! need values to replace
          sol_fc (:,:) = 2.0
          sdnco(:) = 0.85
          j = 0
          j = 1!ihru
        ...
        !! mineralization can occur oj if temp above 0 deg
    ...
    !          sut = Min(1., sut)
              ...
              !! fresh organic n and p (upper two layers oj)
              rmn1 = 0.
              rmp = 0.
              if (k <= 2) then
                r4 = 0.
    ...
    !!  compute denitrification
            wdn = 0.   
         !LZ  if (i_sep(j) /= k .or. isep_opt(j) /= 1) then
             if (sut >= sdnco(j)) then
